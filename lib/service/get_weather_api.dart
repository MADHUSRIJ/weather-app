...
class WeatherApi{
  ...
  static Future<Map<String, dynamic>> getForecastUsingLatLong(String lat, String lon) async{
    String weatherApi = "${weatherUrl}forecast?lat=$lat&lon=$lon&cnt=9&appid=$apiKey";
    try{
      http.Client client = http.Client();
      Map<String, dynamic>? responseBody;
      final response = await client.get(Uri.parse(weatherApi));
